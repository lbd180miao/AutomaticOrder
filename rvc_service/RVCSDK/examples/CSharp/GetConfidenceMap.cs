...
namespace RVC_CSharp
{
    class Program
    {
        ...
        static void Main()
        {
            ...
            //Get Confidence Map
            ConfidenceMap confidenceMap = camera.GetConfidenceMap();

            Size sz = confidenceMap.GetSize();
            int select_x = sz.width / 2;
            int select_y = sz.height / 2;

            IntPtr dataPtr = confidenceMap.GetDataPtr();
            double[] confidenceData = new double[sz.width * sz.height];
            Marshal.Copy(dataPtr, confidenceData, 0, confidenceData.Length);

            double corr_confidence = confidenceData[select_y * sz.width + select_x];
            Console.WriteLine($"The Confidence in xy: ({select_x},{select_y}) is {corr_confidence}");
