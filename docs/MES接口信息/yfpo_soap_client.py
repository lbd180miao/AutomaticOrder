...
class YfpoSoapMesClient(MesClient):
    ...
    def __init__(self, base_url: str, factory_code: str, prod_line_code: str, *,
                 timeout: int = 8, soap_action: str = '', session=None):
        ...
        # 缺省发空 SOAPAction（WCF 通用分发常见做法）；若 WSDL 要求显式 action 再覆盖
    ...
    def get_rack_recipe(self, rack_code: str) -> dict:
        ...
        return {
            ...
            'rack_status': inner.get('RackStatus'),                       # 料架状态枚举(待甲方确认含义)
    ...
    def upload_boxing_result(self, payload: dict) -> dict:
        ...
        return {
            ...
            'is_not_task': inner.get('IsNotTask'),                        # 1=暂无任务/区位(待甲方确认)
